...
**图 5.8** 简单向量加法可用于根据角色当前帧的位置和速度，求出其下一帧的位置。
...
**图 5.9** 球体-球体相交测试只涉及向量减法、向量大小和浮点比较运算。
...
**图 5.15** 在外代数（Grassman 代数）中，一次楔积产生伪向量或双向量，两次楔积产生伪标量或三向量。
...
**图 5.16** 点 **A** 和 **B** 之间的线性插值（lerp），其中 `β = 0.4`。