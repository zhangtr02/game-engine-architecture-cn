...
**Figure 5.18.** 飞机模型空间中 front、left 和 up 轴基向量的一种可能选择。
...
**Figure 5.19.** 一架 Lear 喷气机的左翼尖在模型空间中位于 `(5, 0, 0)`。如果该飞机绕世界空间 y 轴旋转 90 度，并且其模型空间原点被平移到世界空间中的 `(−25, 50, 8)`，那么其左翼尖在世界空间坐标中最终会位于 `(−25, 50, 3)`。
...
**Figure 5.20.** 左手和右手观察空间示例，也称为摄像机空间。
...
**Figure 5.21.** 当子坐标轴相对于父坐标轴旋转角度 `γ` 时的基变换。
...
**Figure 5.22.** 解释变换矩阵的两种方式。左图中，点相对于一组固定坐标轴移动；右图中，坐标轴向相反方向移动，而点保持固定。
