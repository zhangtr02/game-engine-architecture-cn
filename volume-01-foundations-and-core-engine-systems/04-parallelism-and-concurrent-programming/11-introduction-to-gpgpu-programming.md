...
**Figure 4.39.** 典型 GPU 由多个计算单元（compute unit, CU）组成，每个计算单元都像一个精简版 CPU 核心。CU 包含一个取指/译码单元、一个 L1 缓存、可能包含一块本地数据存储 RAM（local data store RAM, LDS）、一个标量 ALU，以及若干用于执行向量化代码的 SIMD 单元。
...
**Figure 4.40.** 每当某个 wavefront 发生停顿时，例如由于内存访问延迟，SIMD 单元就会上下文切换到另一个 wavefront 来填补延迟空槽。
