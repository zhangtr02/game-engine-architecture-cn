...
**Figure 4.20.** 内核和设备驱动程序直接位于硬件之上，并在特权模式下运行。所有其他操作系统软件和用户程序都构建在内核与驱动层之上，并在较受限制的用户模式下运行。
...
**Figure 4.21.** CPU 保护环示例，图中展示了四个环：内核运行在 ring 0，设备驱动程序运行在 ring 1，具有 I/O 权限的受信任程序运行在 ring 2，所有其他用户程序运行在 ring 3。
...
**Figure 4.22.** Apple II 计算机上运行的三个全屏程序。Apple II 上的程序总是全屏的，因为它一次只能运行一个程序。从左到右依次是 Copy II Plus、AppleWorks 文字处理器和 The Locksmith。
...
**Figure 4.23.** 一个进程在 32 位 Windows 下可能呈现的内存映射。
...
**Figure 4.24.** 进程封装运行一个或多个线程所需的资源。每个线程封装一个执行上下文，其中包括 CPU 寄存器内容和调用栈。
...
**Figure 4.25.** 每个线程都可以处于三种状态之一：Running、Runnable 或 Blocked。
...
**Figure 4.26.** Visual Studio 中的 Threads 窗口是调试多线程程序的主要界面。
