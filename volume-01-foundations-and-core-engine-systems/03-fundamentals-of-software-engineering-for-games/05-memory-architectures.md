...
**Table 3.2.** 将 32 位地址空间划分为 4 KiB 页。
...
**Figure 3.22.** 虚拟内存空间中按页大小划分的地址范围，会被重新映射到物理内存页、磁盘上的交换文件，或者保持未映射状态。
...
**Figure 3.23.** MMU 截获一次内存读取操作，并把虚拟地址拆分为虚拟页索引和偏移。虚拟页索引通过页表转换为物理页索引，物理地址则由物理页索引和原始偏移构造而成。最后，该指令使用重映射后的物理地址执行。
...
CPU 与一组 RAM 之间的物理距离也会影响该内存的访问延迟。这是因为电子信号在计算机内部以有限速度传播。理论上，电子信号由电磁波构成，因此传播速度接近光速<sup>7</sup>。但内存访问信号在系统中传输时，还会经过各种开关电路和逻辑电路，这些电路会引入额外延迟。因此，某个内存单元越靠近使用它的 CPU 核心，其访问延迟往往越低。

> **脚注 7**：电子信号在铜线或光纤这样的传输介质中的传播速度总是略低于真空中的光速。每种互连材料都有自己的 characteristic velocity factor（特征速度因子，VF），范围可以从低于真空光速的 50% 到 99%。
...
**Figure 3.24.** CPU 性能与内存性能之间不断扩大的差距称为内存鸿沟。（改编自 [24] John L. Hennessy 与 David A. Patterson 所著 *Computer Architecture: A Quantitative Approach*。）
...
**Figure 3.26.** 缓存中的每一行都关联一个 tag，用来指示相应行来自主 RAM 中哪个缓存大小的块。
...
PlayStation 3 是 NUMA 架构的经典例子。PS3 包含一个主 CPU，称为 Power processing unit（PPU），8 个称为 synergistic processing units（SPU）的协处理器<sup>8</sup>，以及一个 NVIDIA RSX 图形处理单元（GPU）。PPU 可以独占访问 256 MiB 的主系统 RAM（带有 L1 和 L2 缓存）；GPU 可以独占访问 256 MiB 的视频 RAM（VRAM）；每个 SPU 则拥有自己私有的 256 KiB local store。

> **脚注 8**：只有 6 个 SPU 可供游戏应用程序使用——其中 1 个 SPU 保留给操作系统使用，另一个完全禁用，以规避制造过程中不可避免的缺陷。
...
**Figure 3.29.** PS3 Cell Broadband 架构的简化视图。
...
**Figure 3.30.** PS2 内存架构的简化视图，展示了 16 KiB 暂存区（SPR）、主 CPU（EE）的 L1 缓存、两个向量单元（VU0 和 VU1）、图形合成器（GS）可访问的 4 MiB 视频 RAM、32 MiB 主 RAM、DMA 控制器以及系统总线。
